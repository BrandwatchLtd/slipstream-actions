...
test('generates a content hash', async () => {
  const hash = await getHashOfFiles(path.resolve(__dirname, './fixtures'));
  expect(hash).toBe('92a4e078dc6750d10eb2');
});

test('throws an error if content hash genertion fails', async () => {
  const setFailedSpy = jest.spyOn(core, 'setFailed');

  return getHashOfFiles(path.resolve(__dirname, './fixtures/does-not-exist')).catch((error) => {
    expect(error.code).toBe('ENOENT');
    expect(error.path).toBe(path.resolve(__dirname, './fixtures/does-not-exist'));
