...
const exec = __nccwpck_require__(903);
...
const git = __nccwpck_require__(226);
const fs = __nccwpck_require__(147);
const path = __nccwpck_require__(17);
const util = __nccwpck_require__(837);
const core = __nccwpck_require__(199);
const tc = __nccwpck_require__(124);
const exec = __nccwpck_require__(903);
const AWS = __nccwpck_require__(521);
const crypto = __nccwpck_require__(113);
const mime = __nccwpck_require__(400);

const { default: ShortUniqueId } = __nccwpck_require__(570);
const commandExists = __nccwpck_require__(703);
...
function pushFilesToS3Bucket(dir, bucket, pathPrefix) {
  if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
    throw new Error("you must set environment variables: 'AWS_ACCESS_KEY_ID' and 'AWS_SECRET_ACCESS_KEY'");
  }
  ...
  function s3UploadPromise(filePath, relativeTo, Bucket, bucketPath) {
    return fs.promises.readFile(filePath)
      .then((fileData) => {
        ...
        const params = {
          Bucket,
          Body: fileData,
          Key: path.join(bucketPath, relativePath),
  ...
  return Promise.all(files.map((file) => s3UploadPromise(file, absolutePath, bucket, pathPrefix)));
}
...
function walkSync(dir) {
  return fs.readdirSync(dir, { withFileTypes: true })
    .flatMap((file) => {
      if (file.isDirectory()) {
        return walkSync(path.join(dir, file.name));
      }
      return path.join(dir, file.name);
    });
}

async function pushFilesToGCSBucket(files, bucketAddress) {
...
async function pushFilesToBucket(files, bucketAddress) {
  if (bucketAddress.startsWith('gs://')) {
    return pushFilesToGCSBucket(files, bucketAddress);
  }

  if (bucketAddress.startsWith('s3://')) {
    const fullPath = bucketAddress.substr(5);
    const parts = fullPath.split('/');
    const bucket = parts.shift();
    const keyPath = parts.join('/');

    return pushFilesToS3Bucket(files, bucket, keyPath);
  }
  throw new Error('bucket address must start with either gs:// or s3://');
}

async function removeFilesFromGCSBucket(bucketAddress) {
  const args = '-r'; // recursive, no-overwrite, zip
...
async function removeFilesFromS3Bucket(bucket, keyPath) {
  if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
    throw new Error("you must set environment variables: 'AWS_ACCESS_KEY_ID' and 'AWS_SECRET_ACCESS_KEY'");
  }
  ...
  if (listedObjects.IsTruncated) await removeFilesFromS3Bucket(bucket, keyPath);
...
async function removeFilesFromBucket(bucketAddress) {
  if (bucketAddress.startsWith('gs://')) {
    return removeFilesFromGCSBucket(bucketAddress);
  }
  if (bucketAddress.startsWith('s3://')) {
    const fullPath = bucketAddress.substr(5);
    const parts = fullPath.split('/');
    ...
    return removeFilesFromS3Bucket(bucket, keyPath);
  }
  throw new Error('bucket address must start with either gs:// or s3://');
}

function getHashOfFiles(filesDir) {
  let files;
  try {
    files = walkSync(filesDir);
  } catch (e) {
    core.setFailed('Hash generation failed');
    return Promise.reject(e);
  }

  const content = files.reduce((acc, filePath) => {
    core.debug(`Read content of ${filePath}`);
    return acc.then((accData) => fs.promises.readFile(filePath)
      .then((fileData) => accData + fileData.toString('hex')));
  }, Promise.resolve(''));
...
async function gcsDirectoryExists(url) {
...
async function s3DirectoryExists(Bucket, Prefix) {
  if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
    throw new Error("you must set environment variables: 'AWS_ACCESS_KEY_ID' and 'AWS_SECRET_ACCESS_KEY'");
  }
...
async function directoryExists(bucketAddress) {
  if (bucketAddress.startsWith('gs://')) {
    return gcsDirectoryExists(bucketAddress);
  }
  ...
  if (bucketAddress.startsWith('s3://')) {
    ...
    return s3DirectoryExists(bucket, keyPath);
  }
  throw new Error('bucket address must start with either gs:// or s3://');
}
...
async function installSlipstreamCLI(downloadURL) {
  ...
  if (downloadURL.endsWith('.sh')) {
    // run install script
    core.startGroup('Installing the Slipstream CLI (with installer script)');

    const slipstreamInstallerPath = await tc.downloadTool(downloadURL);
...
module.exports = eval("require")("short-unique-id");
...
const push = __nccwpck_require__(349);
